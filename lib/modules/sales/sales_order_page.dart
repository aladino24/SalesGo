...
class _SalesOrderPageState extends State<SalesOrderPage> {
  ...
  double get _total => (_subtotal - _lineDiscount - _discountValue)
  ...
  Future<void> _loadProducts() async {
    _products = await _master.getProducts();
  ...
  Future<void> _addProduct() async {
    if (_products.isEmpty) {
      Get.snackbar(
        ...
        'Unduh data master produk terlebih dahulu.',
      );
      return;
    }
    ...
    final accepted = await Get.dialog<bool>(
      AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Stok tersedia: ${selected.stock}'),
            TextField(
              controller: quantity,
            ...
            TextField(
              ...
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'Diskon item (Rp)'),
            ),
          ],
        ),
        actions: [
    ...
    if (accepted == true && qty > 0 && qty <= selected.stock) {
      setState(
        () => _cart.add(
          SalesOrderItem(
            ...
            discount: double.tryParse(lineDiscount.text) ?? 0,
          ),
        ),
      );
      ...
      Get.snackbar(
        'Jumlah tidak valid',
        'Jumlah maksimal adalah stok tersedia (${selected.stock}).',
      );
    }
    quantity.dispose();
  ...
  Future<void> _submit() async {
    if (_cart.isEmpty) {
      Get.snackbar('Cart kosong', 'Tambahkan minimal satu produk.');
      return;
    }
    ...
    final order = SalesOrderModel(
      ...
      items: _cart,
      total: _total,
      discount: _lineDiscount + _discountValue,
      status: 'Pending Sync',
    ...
    Get.snackbar(
      'Order disimpan',
      'Cart ${_cart.length} produk tersimpan lokal dan akan disinkronkan.',
      ...
      child: Padding(
        ...
        child: FilledButton(
          ...
          child: _saving
              ...
              : Text('Simpan Order • Rp ${_total.toStringAsFixed(0)}'),
        ),
      ),
    ),
    body: _loading
            ...
            children: [
              ...
              if (_cart.isEmpty)
                const SfaEmptyState(
                  ...
                  description:
                      'Pilih produk dari master data untuk membuat order.',
                )
              else
                ..._cart.asMap().entries.map((entry) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text(
                        '${item.quantity} × Rp ${item.unitPrice.toStringAsFixed(0)}${item.discount > 0 ? ' • diskon Rp ${item.discount.toStringAsFixed(0)}' : ''}',
                      ),
                      trailing: IconButton(
                        icon: const Icon(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _summary('Diskon', _lineDiscount + _discountValue),
                      const Divider(),
                      _summary('Total', _total, prominent: true),
                    ],
                  ),
                ),
              ),
  ...
  Widget _summary(String label, double value, {bool prominent = false}) =>
      Padding(
        ...
        child: Row(
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                color: prominent ? AppColors.primary : AppColors.textPrimary,
                fontWeight: prominent ? FontWeight.w800 : FontWeight.w500,
              ),
            ),
          ],

      ...
      return RefreshIndicator(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (_, index) {
            ...
            final period = '${item.startAt.day.toString().padLeft(2, '0')}/${item.startAt.month.toString().padLeft(2, '0')}/${item.startAt.year} - ${item.endAt.day.toString().padLeft(2, '0')}/${item.endAt.month.toString().padLeft(2, '0')}/${item.endAt.year}';
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(item.destination, style: const TextStyle(fontWeight: FontWeight.w800)),
                    Text(item.type == 'out_of_town' ? 'Perjalanan luar kota' : 'Perjalanan dalam kota', style: const TextStyle(fontSize: 11)),
                  ])),
                  SfaStatusChip(label: item.status),
                ]),
                const SizedBox(height: 12),
                Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Theme.of(context).colorScheme.primaryContainer.withValues(alpha: .35), borderRadius: BorderRadius.circular(10)), child: Row(children: [const Icon(Icons.date_range_outlined, size: 18), const SizedBox(width: 8), Expanded(child: Text('Periode perjalanan\n$period', style: const TextStyle(fontSize: 12)))])),
                if (item.approvalStatus != 'Not Required') Padding(padding: const EdgeInsets.only(top: 8), child: Text('Approval: ${item.approvalStatus}', style: const TextStyle(fontSize: 11))),
                const SizedBox(height: 12),
                if (item.status == 'Planned') SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: item.type == 'out_of_town' && item.approvalStatus != 'Approved' ? null : () => _startJourney(item), icon: const Icon(Icons.play_arrow_rounded), label: const Text('Mulai Perjalanan'))),
                if (item.status == 'Active') SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: () => controller.changeStatus(item, 'Completed'), icon: const Icon(Icons.stop_circle_outlined), label: const Text('Akhiri Perjalanan'))),
              ]),
            ));
          },
        ),

...
class CheckOutPage extends StatefulWidget {
  const CheckOutPage({super.key, required this.outlet, required this.visitId});
...
class _CheckOutPageState extends State<CheckOutPage> {
  final _location = LocationService(),
      ...
      _localVisits = VisitLocalDataSource();
  ...
  Future<void> _load() async {
    ...
    try {
      snapshot = await _location.currentLocation();
    } on LocationFailure catch (e) {
      error = e.message;
    } catch (_) {
      error = 'Lokasi tidak dapat diambil.';
    }
  ...
  Future<void> _submit() async {
    if (snapshot == null) {
      SfaFeedbackDialog.show(
        ...
        message: error ?? 'Ambil lokasi terlebih dahulu.',
      );
      return;
    }
    ...
    await Get.find<SyncManager>().queueItem(
      ...
      endpoint: ApiEndpoints.checkOuts,
      method: 'POST',
      uuid: uuid.v4(),
      ...
      payload: {
        ...
        'location': snapshot!.toJson(),
      },
    );
    await VisitTimelineRepository().record(
      outletId: widget.outlet.id,
      visitId: widget.visitId,
      activity: 'check_out',
      description: 'Check-out outlet',
      location: snapshot!.toJson(),
    );
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    final outlet = LatLng(widget.outlet.latitude, widget.outlet.longitude);
    final current = snapshot == null
        ? outlet
        ...
        child: ListView(
          ...
          children: [
            ...
            loading
                ...
                : const Row(
                    children: [
                      Icon(Icons.gps_fixed_rounded, color: AppColors.success),
                      SizedBox(width: 8),
                      Text(
                        'Lokasi checkout tervalidasi',
                        style: TextStyle(
                          fontSize: 12,
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                hintText: 'Tulis ringkasan kunjungan...',
              ),
            ),
            const SizedBox(height: 20),
            FilledButton(
